...
class UserService:
    ...
    async def get_user_by_id(self, id: int, session: AsyncSession):
        ...
        logger.debug("Получен пользователь: id=%s, имя пользователя=%s", user.id, user.username)
        return user

    async def get_user_by_username(self, name: str, session: AsyncSession):
        logger.info("Получение пользователя с именем %s", name)
        user = await self.repository.get_by_username(name, session)
        if user is None:
            logger.warning("Пользователь с именем %s не найден", name)
    ...
    async def get_user_all(self, session: AsyncSession):
        logger.info("Получение всех пользователей")
        users = await self.repository.get_all(session)
        logger.debug("Получено %d пользователей", len(users))
        return users

    async def create_user(self, userCreate: UserCreate, session: AsyncSession):
        try:
            ...
            logger.info("Пользователь создан: id=%s, имя пользователя=%s, email=%s", user.id, user.username, user.email)
            return JSONResponse(
            ...
            logger.error("Непредвиденная ошибка при создании пользователя: %s", str(e))
    ...
    async def update_user(self, id: int, userUpdate: UserUpdate, session: AsyncSession):
        ...
        logger.info("Пользователь обновлён: id=%s, имя пользователя=%s, email=%s", user.id, user.username, user.email)
        return JSONResponse(
    ...
    async def delete_user_by_id(self, id: int, session: AsyncSession):
        ...
        logger.info("Пользователь удалён: id=%s, имя пользователя=%s, email=%s", user.id, user.username, user.email)
        return JSONResponse(
            status_code=status.HTTP_200_OK,
            content={
    ...
    async def ban_user_by_id(self, id: int, session: AsyncSession):
        ...
        logger.info("Пользователь заблокирован: id=%s, имя пользователя=%s, email=%s", user.id, user.username, user.email)
        return JSONResponse(
            status_code=status.HTTP_200_OK,
            content={
